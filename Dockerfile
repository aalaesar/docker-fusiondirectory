...
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y gnupg2 apt-transport-https && \
...
RUN apt-get update && apt-get download fusiondirectory && dpkg-deb -x ./fusiondirectory*.deb /tmp && \
